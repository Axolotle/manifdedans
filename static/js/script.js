window.onload = () => {
    // Use a "/test" namespace.
    // An application can open a connection on multiple namespaces, and
    // Socket.IO will multiplex all those connections on a single
    // physical channel. If you don't care about multiple channels, you
    // can set the namespace to an empty string.
    var namespace = '/test';

    // Connect to the Socket.IO server.
    // The connection URL has the following format, relative to the current page:
    //     http[s]://<domain>:<port>[/<namespace>]
    var socket = io(namespace);

    // Event handler for new connections.
    socket.on('connect', function() {
        socket.emit('log', {data: 'I\'m connected!'});
    });

    socket.on('connection_ok', on_connection_ok);
    socket.on('log', on_log);
    socket.on('message_changed', on_message_changed);
    socket.on('message_removed', on_message_removed);

    document.getElementById('participate').onclick = e => {
        let participation = new Participation(e.target);
        participation.on_emit_message = msg => {
            socket.emit('message_changed', {data: msg});
            document.getElementById('stop').classList.remove('hidden');
            document.getElementById('goto').classList.remove('hidden');
        };
        return false;
    };
    document.getElementById('goto').onclick = goto_self;

    document.querySelector('#stop').onclick = () => {
        socket.emit('stop_participation');
        document.getElementById('stop').classList.add('hidden');
        document.getElementById('goto').classList.add('hidden');
        document.getElementById('participate').focus();
        return false;
    };
};


// ╭───────────────────────────╮
// │ SOCKET RESPONSES HANDLERS │
// ╰───────────────────────────╯

function on_connection_ok(rsp) {
    let container = document.getElementById('manif');
    for (let id in rsp.data) {
        let elem = add_new_message(id, rsp.data[id]);
        container.appendChild(elem);
    }
    // reference self id in dataset
    container.dataset.self = rsp.id;
}

function on_message_changed(rsp) {
    let elem = document.getElementById('p' + rsp.id);
    if (!elem) {
        elem = add_new_message(rsp.id, rsp.data);
        document.getElementById('manif').appendChild(elem);
    } else {
        modify_message(elem, rsp.data);
    }
}

function on_message_removed(rsp) {
    let elem = document.getElementById('p' + rsp.id);
    if (elem) {
        elem.parentElement.removeChild(elem);
    }
}

function on_log(rsp) {
    let p = document.createElement('p');
    p.textContent = 'Connected as: ' + rsp.id.substring(0, 5)
        + ' / log: ' + rsp.data;
    document.getElementById('log').appendChild(p);
}


// ╭──────────────────╮
// │ MESSAGES HELPERS │
// ╰──────────────────╯

function add_new_message(id, msg) {
    let elem = document.createElement('DIV');
    elem.id = 'p' + id;
    modify_message(elem, msg);
    return elem;
}

function modify_message(elem, msg) {
    if (is_single_emoji(msg)) {
        elem.classList.remove('slogan');
        elem.textContent = msg;
    } else {
        elem.classList.add('slogan');
        if (elem.firstElementChild) {
            elem.firstElementChild.textContent = msg;
        } else {
            let p = document.createElement('P');
            p.textContent = msg;
            if (elem.firstChild) {
                elem.replaceChild(p, elem.firstChild)
            } else {
                elem.appendChild(p);
            }
        }
    }
}


// ╭───────╮
// │ UTILS │
// ╰───────╯

function is_single_emoji(msg) {
    // Checks if the message's content includes a single emoji
    // The regex is from https://mths.be/emoji-regex, by Mathias Bynens, MIT license
    let match = msg.match(/\u{1F3F4}\u{E0067}\u{E0062}(?:\u{E0077}\u{E006C}\u{E0073}|\u{E0073}\u{E0063}\u{E0074}|\u{E0065}\u{E006E}\u{E0067})\u{E007F}|\u{1F469}\u200D\u{1F469}\u200D(?:\u{1F466}\u200D\u{1F466}|\u{1F467}\u200D[\u{1F466}\u{1F467}])|\u{1F468}(?:\u{1F3FF}\u200D(?:\u{1F91D}\u200D\u{1F468}[\u{1F3FB}-\u{1F3FE}]|[\u{1F33E}\u{1F373}\u{1F37C}\u{1F393}\u{1F3A4}\u{1F3A8}\u{1F3EB}\u{1F3ED}\u{1F4BB}\u{1F4BC}\u{1F527}\u{1F52C}\u{1F680}\u{1F692}\u{1F9AF}-\u{1F9B3}\u{1F9BC}\u{1F9BD}])|\u{1F3FE}\u200D(?:\u{1F91D}\u200D\u{1F468}[\u{1F3FB}-\u{1F3FD}\u{1F3FF}]|[\u{1F33E}\u{1F373}\u{1F37C}\u{1F393}\u{1F3A4}\u{1F3A8}\u{1F3EB}\u{1F3ED}\u{1F4BB}\u{1F4BC}\u{1F527}\u{1F52C}\u{1F680}\u{1F692}\u{1F9AF}-\u{1F9B3}\u{1F9BC}\u{1F9BD}])|\u{1F3FD}\u200D(?:\u{1F91D}\u200D\u{1F468}[\u{1F3FB}\u{1F3FC}\u{1F3FE}\u{1F3FF}]|[\u{1F33E}\u{1F373}\u{1F37C}\u{1F393}\u{1F3A4}\u{1F3A8}\u{1F3EB}\u{1F3ED}\u{1F4BB}\u{1F4BC}\u{1F527}\u{1F52C}\u{1F680}\u{1F692}\u{1F9AF}-\u{1F9B3}\u{1F9BC}\u{1F9BD}])|\u{1F3FC}\u200D(?:\u{1F91D}\u200D\u{1F468}[\u{1F3FB}\u{1F3FD}-\u{1F3FF}]|[\u{1F33E}\u{1F373}\u{1F37C}\u{1F393}\u{1F3A4}\u{1F3A8}\u{1F3EB}\u{1F3ED}\u{1F4BB}\u{1F4BC}\u{1F527}\u{1F52C}\u{1F680}\u{1F692}\u{1F9AF}-\u{1F9B3}\u{1F9BC}\u{1F9BD}])|\u{1F3FB}\u200D(?:\u{1F91D}\u200D\u{1F468}[\u{1F3FC}-\u{1F3FF}]|[\u{1F33E}\u{1F373}\u{1F37C}\u{1F393}\u{1F3A4}\u{1F3A8}\u{1F3EB}\u{1F3ED}\u{1F4BB}\u{1F4BC}\u{1F527}\u{1F52C}\u{1F680}\u{1F692}\u{1F9AF}-\u{1F9B3}\u{1F9BC}\u{1F9BD}])|\u200D(?:\u2764\uFE0F\u200D(?:\u{1F48B}\u200D)?\u{1F468}|[\u{1F468}\u{1F469}]\u200D(?:\u{1F466}\u200D\u{1F466}|\u{1F467}\u200D[\u{1F466}\u{1F467}])|\u{1F466}\u200D\u{1F466}|\u{1F467}\u200D[\u{1F466}\u{1F467}]|[\u{1F468}\u{1F469}]\u200D[\u{1F466}\u{1F467}]|[\u2695\u2696\u2708]\uFE0F|[\u{1F466}\u{1F467}]|[\u{1F33E}\u{1F373}\u{1F37C}\u{1F393}\u{1F3A4}\u{1F3A8}\u{1F3EB}\u{1F3ED}\u{1F4BB}\u{1F4BC}\u{1F527}\u{1F52C}\u{1F680}\u{1F692}\u{1F9AF}-\u{1F9B3}\u{1F9BC}\u{1F9BD}])|(?:\u{1F3FF}\u200D[\u2695\u2696\u2708]|\u{1F3FE}\u200D[\u2695\u2696\u2708]|\u{1F3FD}\u200D[\u2695\u2696\u2708]|\u{1F3FC}\u200D[\u2695\u2696\u2708]|\u{1F3FB}\u200D[\u2695\u2696\u2708])\uFE0F|[\u{1F3FB}-\u{1F3FF}])|\u{1F9D1}(?:[\u{1F3FB}-\u{1F3FF}]\u200D(?:\u{1F91D}\u200D\u{1F9D1}[\u{1F3FB}-\u{1F3FF}]|[\u{1F33E}\u{1F373}\u{1F37C}\u{1F384}\u{1F393}\u{1F3A4}\u{1F3A8}\u{1F3EB}\u{1F3ED}\u{1F4BB}\u{1F4BC}\u{1F527}\u{1F52C}\u{1F680}\u{1F692}\u{1F9AF}-\u{1F9B3}\u{1F9BC}\u{1F9BD}])|\u200D(?:\u{1F91D}\u200D\u{1F9D1}|[\u{1F33E}\u{1F373}\u{1F37C}\u{1F384}\u{1F393}\u{1F3A4}\u{1F3A8}\u{1F3EB}\u{1F3ED}\u{1F4BB}\u{1F4BC}\u{1F527}\u{1F52C}\u{1F680}\u{1F692}\u{1F9AF}-\u{1F9B3}\u{1F9BC}\u{1F9BD}]))|\u{1F469}(?:\u200D(?:\u2764\uFE0F\u200D(?:\u{1F48B}\u200D[\u{1F468}\u{1F469}]|[\u{1F468}\u{1F469}])|[\u{1F33E}\u{1F373}\u{1F37C}\u{1F393}\u{1F3A4}\u{1F3A8}\u{1F3EB}\u{1F3ED}\u{1F4BB}\u{1F4BC}\u{1F527}\u{1F52C}\u{1F680}\u{1F692}\u{1F9AF}-\u{1F9B3}\u{1F9BC}\u{1F9BD}])|\u{1F3FF}\u200D[\u{1F33E}\u{1F373}\u{1F37C}\u{1F393}\u{1F3A4}\u{1F3A8}\u{1F3EB}\u{1F3ED}\u{1F4BB}\u{1F4BC}\u{1F527}\u{1F52C}\u{1F680}\u{1F692}\u{1F9AF}-\u{1F9B3}\u{1F9BC}\u{1F9BD}]|\u{1F3FE}\u200D[\u{1F33E}\u{1F373}\u{1F37C}\u{1F393}\u{1F3A4}\u{1F3A8}\u{1F3EB}\u{1F3ED}\u{1F4BB}\u{1F4BC}\u{1F527}\u{1F52C}\u{1F680}\u{1F692}\u{1F9AF}-\u{1F9B3}\u{1F9BC}\u{1F9BD}]|\u{1F3FD}\u200D[\u{1F33E}\u{1F373}\u{1F37C}\u{1F393}\u{1F3A4}\u{1F3A8}\u{1F3EB}\u{1F3ED}\u{1F4BB}\u{1F4BC}\u{1F527}\u{1F52C}\u{1F680}\u{1F692}\u{1F9AF}-\u{1F9B3}\u{1F9BC}\u{1F9BD}]|\u{1F3FC}\u200D[\u{1F33E}\u{1F373}\u{1F37C}\u{1F393}\u{1F3A4}\u{1F3A8}\u{1F3EB}\u{1F3ED}\u{1F4BB}\u{1F4BC}\u{1F527}\u{1F52C}\u{1F680}\u{1F692}\u{1F9AF}-\u{1F9B3}\u{1F9BC}\u{1F9BD}]|\u{1F3FB}\u200D[\u{1F33E}\u{1F373}\u{1F37C}\u{1F393}\u{1F3A4}\u{1F3A8}\u{1F3EB}\u{1F3ED}\u{1F4BB}\u{1F4BC}\u{1F527}\u{1F52C}\u{1F680}\u{1F692}\u{1F9AF}-\u{1F9B3}\u{1F9BC}\u{1F9BD}])|\u{1F469}\u{1F3FF}\u200D\u{1F91D}\u200D[\u{1F468}\u{1F469}][\u{1F3FB}-\u{1F3FE}]|\u{1F469}\u{1F3FE}\u200D\u{1F91D}\u200D[\u{1F468}\u{1F469}][\u{1F3FB}-\u{1F3FD}\u{1F3FF}]|\u{1F469}\u{1F3FD}\u200D\u{1F91D}\u200D[\u{1F468}\u{1F469}][\u{1F3FB}\u{1F3FC}\u{1F3FE}\u{1F3FF}]|\u{1F469}\u{1F3FC}\u200D\u{1F91D}\u200D[\u{1F468}\u{1F469}][\u{1F3FB}\u{1F3FD}-\u{1F3FF}]|\u{1F469}\u{1F3FB}\u200D\u{1F91D}\u200D[\u{1F468}\u{1F469}][\u{1F3FC}-\u{1F3FF}]|\u{1F469}\u200D\u{1F466}\u200D\u{1F466}|\u{1F469}\u200D\u{1F469}\u200D[\u{1F466}\u{1F467}]|(?:\u{1F441}\uFE0F\u200D\u{1F5E8}|\u{1F469}(?:\u{1F3FF}\u200D[\u2695\u2696\u2708]|\u{1F3FE}\u200D[\u2695\u2696\u2708]|\u{1F3FD}\u200D[\u2695\u2696\u2708]|\u{1F3FC}\u200D[\u2695\u2696\u2708]|\u{1F3FB}\u200D[\u2695\u2696\u2708]|\u200D[\u2695\u2696\u2708])|\u{1F3F3}\uFE0F\u200D\u26A7|\u{1F9D1}(?:[\u{1F3FB}-\u{1F3FF}]\u200D[\u2695\u2696\u2708]|\u200D[\u2695\u2696\u2708])|\u{1F43B}\u200D\u2744|[\u{1F3C3}\u{1F3C4}\u{1F3CA}\u{1F46E}\u{1F470}\u{1F471}\u{1F473}\u{1F477}\u{1F481}\u{1F482}\u{1F486}\u{1F487}\u{1F645}-\u{1F647}\u{1F64B}\u{1F64D}\u{1F64E}\u{1F6A3}\u{1F6B4}-\u{1F6B6}\u{1F926}\u{1F935}\u{1F937}-\u{1F939}\u{1F93D}\u{1F93E}\u{1F9B8}\u{1F9B9}\u{1F9CD}-\u{1F9CF}\u{1F9D6}-\u{1F9DD}][\u{1F3FB}-\u{1F3FF}]\u200D[\u2640\u2642]|[\u26F9\u{1F3CB}\u{1F3CC}\u{1F575}](?:\uFE0F\u200D[\u2640\u2642]|[\u{1F3FB}-\u{1F3FF}]\u200D[\u2640\u2642])|\u{1F3F4}\u200D\u2620|[\u{1F3C3}\u{1F3C4}\u{1F3CA}\u{1F46E}-\u{1F471}\u{1F473}\u{1F477}\u{1F481}\u{1F482}\u{1F486}\u{1F487}\u{1F645}-\u{1F647}\u{1F64B}\u{1F64D}\u{1F64E}\u{1F6A3}\u{1F6B4}-\u{1F6B6}\u{1F926}\u{1F935}\u{1F937}-\u{1F939}\u{1F93C}-\u{1F93E}\u{1F9B8}\u{1F9B9}\u{1F9CD}-\u{1F9CF}\u{1F9D6}-\u{1F9DF}]\u200D[\u2640\u2642])\uFE0F|\u{1F469}\u200D\u{1F467}\u200D[\u{1F466}\u{1F467}]|\u{1F3F3}\uFE0F\u200D\u{1F308}|\u{1F469}\u200D\u{1F467}|\u{1F469}\u200D\u{1F466}|\u{1F415}\u200D\u{1F9BA}|\u{1F1FD}\u{1F1F0}|\u{1F1F6}\u{1F1E6}|\u{1F1F4}\u{1F1F2}|\u{1F408}\u200D\u2B1B|\u{1F9D1}[\u{1F3FB}-\u{1F3FF}]|\u{1F469}[\u{1F3FB}-\u{1F3FF}]|\u{1F1FF}[\u{1F1E6}\u{1F1F2}\u{1F1FC}]|\u{1F1FE}[\u{1F1EA}\u{1F1F9}]|\u{1F1FC}[\u{1F1EB}\u{1F1F8}]|\u{1F1FB}[\u{1F1E6}\u{1F1E8}\u{1F1EA}\u{1F1EC}\u{1F1EE}\u{1F1F3}\u{1F1FA}]|\u{1F1FA}[\u{1F1E6}\u{1F1EC}\u{1F1F2}\u{1F1F3}\u{1F1F8}\u{1F1FE}\u{1F1FF}]|\u{1F1F9}[\u{1F1E6}\u{1F1E8}\u{1F1E9}\u{1F1EB}-\u{1F1ED}\u{1F1EF}-\u{1F1F4}\u{1F1F7}\u{1F1F9}\u{1F1FB}\u{1F1FC}\u{1F1FF}]|\u{1F1F8}[\u{1F1E6}-\u{1F1EA}\u{1F1EC}-\u{1F1F4}\u{1F1F7}-\u{1F1F9}\u{1F1FB}\u{1F1FD}-\u{1F1FF}]|\u{1F1F7}[\u{1F1EA}\u{1F1F4}\u{1F1F8}\u{1F1FA}\u{1F1FC}]|\u{1F1F5}[\u{1F1E6}\u{1F1EA}-\u{1F1ED}\u{1F1F0}-\u{1F1F3}\u{1F1F7}-\u{1F1F9}\u{1F1FC}\u{1F1FE}]|\u{1F1F3}[\u{1F1E6}\u{1F1E8}\u{1F1EA}-\u{1F1EC}\u{1F1EE}\u{1F1F1}\u{1F1F4}\u{1F1F5}\u{1F1F7}\u{1F1FA}\u{1F1FF}]|\u{1F1F2}[\u{1F1E6}\u{1F1E8}-\u{1F1ED}\u{1F1F0}-\u{1F1FF}]|\u{1F1F1}[\u{1F1E6}-\u{1F1E8}\u{1F1EE}\u{1F1F0}\u{1F1F7}-\u{1F1FB}\u{1F1FE}]|\u{1F1F0}[\u{1F1EA}\u{1F1EC}-\u{1F1EE}\u{1F1F2}\u{1F1F3}\u{1F1F5}\u{1F1F7}\u{1F1FC}\u{1F1FE}\u{1F1FF}]|\u{1F1EF}[\u{1F1EA}\u{1F1F2}\u{1F1F4}\u{1F1F5}]|\u{1F1EE}[\u{1F1E8}-\u{1F1EA}\u{1F1F1}-\u{1F1F4}\u{1F1F6}-\u{1F1F9}]|\u{1F1ED}[\u{1F1F0}\u{1F1F2}\u{1F1F3}\u{1F1F7}\u{1F1F9}\u{1F1FA}]|\u{1F1EC}[\u{1F1E6}\u{1F1E7}\u{1F1E9}-\u{1F1EE}\u{1F1F1}-\u{1F1F3}\u{1F1F5}-\u{1F1FA}\u{1F1FC}\u{1F1FE}]|\u{1F1EB}[\u{1F1EE}-\u{1F1F0}\u{1F1F2}\u{1F1F4}\u{1F1F7}]|\u{1F1EA}[\u{1F1E6}\u{1F1E8}\u{1F1EA}\u{1F1EC}\u{1F1ED}\u{1F1F7}-\u{1F1FA}]|\u{1F1E9}[\u{1F1EA}\u{1F1EC}\u{1F1EF}\u{1F1F0}\u{1F1F2}\u{1F1F4}\u{1F1FF}]|\u{1F1E8}[\u{1F1E6}\u{1F1E8}\u{1F1E9}\u{1F1EB}-\u{1F1EE}\u{1F1F0}-\u{1F1F5}\u{1F1F7}\u{1F1FA}-\u{1F1FF}]|\u{1F1E7}[\u{1F1E6}\u{1F1E7}\u{1F1E9}-\u{1F1EF}\u{1F1F1}-\u{1F1F4}\u{1F1F6}-\u{1F1F9}\u{1F1FB}\u{1F1FC}\u{1F1FE}\u{1F1FF}]|\u{1F1E6}[\u{1F1E8}-\u{1F1EC}\u{1F1EE}\u{1F1F1}\u{1F1F2}\u{1F1F4}\u{1F1F6}-\u{1F1FA}\u{1F1FC}\u{1F1FD}\u{1F1FF}]|[#\*0-9]\uFE0F\u20E3|[\u{1F3C3}\u{1F3C4}\u{1F3CA}\u{1F46E}\u{1F470}\u{1F471}\u{1F473}\u{1F477}\u{1F481}\u{1F482}\u{1F486}\u{1F487}\u{1F645}-\u{1F647}\u{1F64B}\u{1F64D}\u{1F64E}\u{1F6A3}\u{1F6B4}-\u{1F6B6}\u{1F926}\u{1F935}\u{1F937}-\u{1F939}\u{1F93D}\u{1F93E}\u{1F9B8}\u{1F9B9}\u{1F9CD}-\u{1F9CF}\u{1F9D6}-\u{1F9DD}][\u{1F3FB}-\u{1F3FF}]|[\u26F9\u{1F3CB}\u{1F3CC}\u{1F575}][\u{1F3FB}-\u{1F3FF}]|[\u261D\u270A-\u270D\u{1F385}\u{1F3C2}\u{1F3C7}\u{1F442}\u{1F443}\u{1F446}-\u{1F450}\u{1F466}\u{1F467}\u{1F46B}-\u{1F46D}\u{1F472}\u{1F474}-\u{1F476}\u{1F478}\u{1F47C}\u{1F483}\u{1F485}\u{1F4AA}\u{1F574}\u{1F57A}\u{1F590}\u{1F595}\u{1F596}\u{1F64C}\u{1F64F}\u{1F6C0}\u{1F6CC}\u{1F90C}\u{1F90F}\u{1F918}-\u{1F91C}\u{1F91E}\u{1F91F}\u{1F930}-\u{1F934}\u{1F936}\u{1F977}\u{1F9B5}\u{1F9B6}\u{1F9BB}\u{1F9D2}-\u{1F9D5}][\u{1F3FB}-\u{1F3FF}]|[\u231A\u231B\u23E9-\u23EC\u23F0\u23F3\u25FD\u25FE\u2614\u2615\u2648-\u2653\u267F\u2693\u26A1\u26AA\u26AB\u26BD\u26BE\u26C4\u26C5\u26CE\u26D4\u26EA\u26F2\u26F3\u26F5\u26FA\u26FD\u2705\u270A\u270B\u2728\u274C\u274E\u2753-\u2755\u2757\u2795-\u2797\u27B0\u27BF\u2B1B\u2B1C\u2B50\u2B55\u{1F004}\u{1F0CF}\u{1F18E}\u{1F191}-\u{1F19A}\u{1F1E6}-\u{1F1FF}\u{1F201}\u{1F21A}\u{1F22F}\u{1F232}-\u{1F236}\u{1F238}-\u{1F23A}\u{1F250}\u{1F251}\u{1F300}-\u{1F320}\u{1F32D}-\u{1F335}\u{1F337}-\u{1F37C}\u{1F37E}-\u{1F393}\u{1F3A0}-\u{1F3CA}\u{1F3CF}-\u{1F3D3}\u{1F3E0}-\u{1F3F0}\u{1F3F4}\u{1F3F8}-\u{1F43E}\u{1F440}\u{1F442}-\u{1F4FC}\u{1F4FF}-\u{1F53D}\u{1F54B}-\u{1F54E}\u{1F550}-\u{1F567}\u{1F57A}\u{1F595}\u{1F596}\u{1F5A4}\u{1F5FB}-\u{1F64F}\u{1F680}-\u{1F6C5}\u{1F6CC}\u{1F6D0}-\u{1F6D2}\u{1F6D5}-\u{1F6D7}\u{1F6EB}\u{1F6EC}\u{1F6F4}-\u{1F6FC}\u{1F7E0}-\u{1F7EB}\u{1F90C}-\u{1F93A}\u{1F93C}-\u{1F945}\u{1F947}-\u{1F978}\u{1F97A}-\u{1F9CB}\u{1F9CD}-\u{1F9FF}\u{1FA70}-\u{1FA74}\u{1FA78}-\u{1FA7A}\u{1FA80}-\u{1FA86}\u{1FA90}-\u{1FAA8}\u{1FAB0}-\u{1FAB6}\u{1FAC0}-\u{1FAC2}\u{1FAD0}-\u{1FAD6}]|[#\*0-9\xA9\xAE\u203C\u2049\u2122\u2139\u2194-\u2199\u21A9\u21AA\u231A\u231B\u2328\u23CF\u23E9-\u23F3\u23F8-\u23FA\u24C2\u25AA\u25AB\u25B6\u25C0\u25FB-\u25FE\u2600-\u2604\u260E\u2611\u2614\u2615\u2618\u261D\u2620\u2622\u2623\u2626\u262A\u262E\u262F\u2638-\u263A\u2640\u2642\u2648-\u2653\u265F\u2660\u2663\u2665\u2666\u2668\u267B\u267E\u267F\u2692-\u2697\u2699\u269B\u269C\u26A0\u26A1\u26A7\u26AA\u26AB\u26B0\u26B1\u26BD\u26BE\u26C4\u26C5\u26C8\u26CE\u26CF\u26D1\u26D3\u26D4\u26E9\u26EA\u26F0-\u26F5\u26F7-\u26FA\u26FD\u2702\u2705\u2708-\u270D\u270F\u2712\u2714\u2716\u271D\u2721\u2728\u2733\u2734\u2744\u2747\u274C\u274E\u2753-\u2755\u2757\u2763\u2764\u2795-\u2797\u27A1\u27B0\u27BF\u2934\u2935\u2B05-\u2B07\u2B1B\u2B1C\u2B50\u2B55\u3030\u303D\u3297\u3299\u{1F004}\u{1F0CF}\u{1F170}\u{1F171}\u{1F17E}\u{1F17F}\u{1F18E}\u{1F191}-\u{1F19A}\u{1F1E6}-\u{1F1FF}\u{1F201}\u{1F202}\u{1F21A}\u{1F22F}\u{1F232}-\u{1F23A}\u{1F250}\u{1F251}\u{1F300}-\u{1F321}\u{1F324}-\u{1F393}\u{1F396}\u{1F397}\u{1F399}-\u{1F39B}\u{1F39E}-\u{1F3F0}\u{1F3F3}-\u{1F3F5}\u{1F3F7}-\u{1F4FD}\u{1F4FF}-\u{1F53D}\u{1F549}-\u{1F54E}\u{1F550}-\u{1F567}\u{1F56F}\u{1F570}\u{1F573}-\u{1F57A}\u{1F587}\u{1F58A}-\u{1F58D}\u{1F590}\u{1F595}\u{1F596}\u{1F5A4}\u{1F5A5}\u{1F5A8}\u{1F5B1}\u{1F5B2}\u{1F5BC}\u{1F5C2}-\u{1F5C4}\u{1F5D1}-\u{1F5D3}\u{1F5DC}-\u{1F5DE}\u{1F5E1}\u{1F5E3}\u{1F5E8}\u{1F5EF}\u{1F5F3}\u{1F5FA}-\u{1F64F}\u{1F680}-\u{1F6C5}\u{1F6CB}-\u{1F6D2}\u{1F6D5}-\u{1F6D7}\u{1F6E0}-\u{1F6E5}\u{1F6E9}\u{1F6EB}\u{1F6EC}\u{1F6F0}\u{1F6F3}-\u{1F6FC}\u{1F7E0}-\u{1F7EB}\u{1F90C}-\u{1F93A}\u{1F93C}-\u{1F945}\u{1F947}-\u{1F978}\u{1F97A}-\u{1F9CB}\u{1F9CD}-\u{1F9FF}\u{1FA70}-\u{1FA74}\u{1FA78}-\u{1FA7A}\u{1FA80}-\u{1FA86}\u{1FA90}-\u{1FAA8}\u{1FAB0}-\u{1FAB6}\u{1FAC0}-\u{1FAC2}\u{1FAD0}-\u{1FAD6}]\uFE0F?|[\u261D\u26F9\u270A-\u270D\u{1F385}\u{1F3C2}-\u{1F3C4}\u{1F3C7}\u{1F3CA}-\u{1F3CC}\u{1F442}\u{1F443}\u{1F446}-\u{1F450}\u{1F466}-\u{1F478}\u{1F47C}\u{1F481}-\u{1F483}\u{1F485}-\u{1F487}\u{1F48F}\u{1F491}\u{1F4AA}\u{1F574}\u{1F575}\u{1F57A}\u{1F590}\u{1F595}\u{1F596}\u{1F645}-\u{1F647}\u{1F64B}-\u{1F64F}\u{1F6A3}\u{1F6B4}-\u{1F6B6}\u{1F6C0}\u{1F6CC}\u{1F90C}\u{1F90F}\u{1F918}-\u{1F91F}\u{1F926}\u{1F930}-\u{1F939}\u{1F93C}-\u{1F93E}\u{1F977}\u{1F9B5}\u{1F9B6}\u{1F9B8}\u{1F9B9}\u{1F9BB}\u{1F9CD}-\u{1F9CF}\u{1F9D1}-\u{1F9DD}]/gu);
    if (match && match[0] === msg) {
        return true;
    }
    return false;
}

function goto_self() {
    let self_id = document.getElementById('manif').dataset.self;
    document.getElementById('p' + self_id).scrollIntoView();
    return false;
}


// ╭────╮
// │ UI │
// ╰────╯

class Dialog {
    constructor(container, dialog, lastFocused) {
        this.container = container;
        this.dialog = dialog;
        this.lastFocused = lastFocused;

        this.trap_focus = this.trap_focus.bind(this);
        this.on_container_click = this.on_container_click.bind(this);
        this.on_keyup = this.on_keyup.bind(this);
    }

    open() {
        const cont = this.container;
        cont.classList.remove('hidden');
        cont.addEventListener('focus', this.trap_focus, true);
        cont.addEventListener('click', this.on_container_click, true);
        cont.addEventListener('keyup', this.on_keyup, false);
    }

    close() {
        const cont = this.container;
        cont.classList.add('hidden');
        cont.removeEventListener('focus', this.trap_focus, true);
        cont.removeEventListener('click', this.on_container_click, true);
        cont.removeEventListener('keyup', this.on_keyup, false);

        if (this.lastFocused) {
            this.lastFocused.focus();
        }
    }

    on_container_click(e) {
        if (!this.dialog.contains(e.target)) {
            this.close();
        }
    }

    on_keyup(e) {
        if (e.key === 'Escape') {
            this.close();
        }
    }
}


class Participation extends Dialog {
    constructor(lastFocused) {
        super(document.getElementById('form-parent'),
            document.getElementById('form'), lastFocused);

        this.on_emoji_input = this.on_emoji_input.bind(this);
        this.on_open_emoji_picker = this.on_open_emoji_picker.bind(this);
        this.on_submit = this.on_submit.bind(this);

        this.open();
    }

    open() {
        super.open();

        const emoji_input = document.getElementById('emoji-input');
        emoji_input.addEventListener('input', this.on_emoji_input, true);

        this.dialog.querySelectorAll('button.emoji-picker').forEach(button => {
            button.addEventListener('click', this.on_open_emoji_picker, false);
        });

        this.dialog.querySelectorAll('form').forEach(form => {
            form.addEventListener('submit', this.on_submit, false);
        });

        this.dialog.querySelector('input, button').focus();
    }

    close() {
        super.close();

        const emoji_input = document.getElementById('emoji-input');
        emoji_input.removeEventListener('input', this.on_emoji_input, true);

        this.dialog.querySelectorAll('button.emoji-picker').forEach(button => {
            button.removeEventListener('click', this.on_open_emoji_picker, false);
        });
    }

    on_open_emoji_picker(e) {
        const input = e.target.previousElementSibling;
        const picker = new EmojiPicker(input);
        picker.on_choose = function(emoji) {
            if (input.nodeName === 'INPUT') {
                input.value = emoji;
            } else {
                input.value += emoji;
            }
            picker.close();
        };
    }

    on_emoji_input(e) {
        const emoji_input = e.target;
        if (is_single_emoji(emoji_input.value)) {
            emoji_input.setCustomValidity('');
        } else {
            emoji_input.setCustomValidity('Veuillez entrer un seul émoji');
        }
    }

    on_submit(e) {
        const input = e.target.querySelector('textarea')
            || e.target.querySelector('input');
        const msg = input.value.trim();
        e.preventDefault();
        if (msg !== ''){
            this.on_emit_message(msg);
            this.close();
        }
    }

    trap_focus(e) {
        if (!this.dialog.contains(e.target)) {
            const elems = this.dialog.querySelectorAll('input, button');
            if (e.target.nextElementSibling !== null) {
                elems[elems.length - 1].focus();
            } else {
                elems[0].focus();
            }
        }
    }

    on_emit_message(e) {
        // overide this method to get the chosen message
    }
}


class EmojiDialog extends Dialog {
    constructor(container, dialog, lastFocused) {
        super(container, dialog, lastFocused);

        this.on_click = this.on_click.bind(this);
        this.on_emoji_keydown = this.on_emoji_keydown.bind(this);
        this.on_emoji_keyup = this.on_emoji_keyup.bind(this);
    }

    open() {
        super.open();

        this.dialog.addEventListener('click', this.on_click, false);
    }

    close() {
        super.close();

        this.dialog.removeEventListener('click', this.on_click, false);
    }

    on_emoji_keydown(e) {
        let key = e.key;
        switch (key) {
            case 'Home':
                e.target.parentElement.firstElementChild.focus();
                e.preventDefault();
                break;
            case 'End':
                e.target.parentElement.lastElementChild.focus();
                e.preventDefault();
                break;
        }
    }

    on_emoji_keyup(e) {
        let key = e.key;
        switch (key) {
            case 'ArrowLeft':
                let previous = e.target.previousElementSibling
                ||  e.target.parentElement.lastElementChild;
                previous.focus();
                break;
            case 'ArrowRight':
                let next = e.target.nextElementSibling
                    || e.target.parentElement.firstElementChild;
                next.focus();
                break;
        }
    }

    on_choose(emoji) {
        // overide this method to get the chosen emoji
    }
}


class EmojiPicker extends EmojiDialog {
    constructor(lastFocused) {
        super(document.getElementById('emoji-picker-parent'),
            document.getElementById('emoji-picker'), lastFocused);

        this.on_tab_click = this.on_tab_click.bind(this);
        this.on_tab_keydown = this.on_tab_keydown.bind(this);
        this.on_tab_keyup = this.on_tab_keyup.bind(this);

        this.open();
    }

    get active_tab() {
        return document.querySelector(
            '#emoji-picker [role="tab"][aria-selected="true"]');
    }

    open() {
        super.open();

        this.dialog.querySelectorAll('[role="tab"]').forEach(tab => {
            tab.addEventListener('click', this.on_tab_click, false);
            tab.addEventListener('keydown', this.on_tab_keydown, false);
            tab.addEventListener('keyup', this.on_tab_keyup, false);
        });
        this.dialog.querySelectorAll('[role="tabpanel"]').forEach(panel => {
            panel.addEventListener('keydown', this.on_emoji_keydown, true);
            panel.addEventListener('keyup', this.on_emoji_keyup, false);
        });

        this.dialog.querySelector('[role="tab"][aria-selected="true"]').focus();
    }

    close() {
        super.close();

        this.dialog.removeEventListener('click', this.on_click, false);
        this.dialog.querySelectorAll('[role="tab"]').forEach(tab => {
            tab.removeEventListener('click', this.on_tab_click, false);
            tab.removeEventListener('keydown', this.on_tab_keydown, false);
            tab.removeEventListener('keyup', this.on_tab_keyup, false);
        });
        this.dialog.querySelectorAll('[role="tabpanel"]').forEach(panel => {
            panel.removeEventListener('keydown', this.on_emoji_keydown, true);
            panel.removeEventListener('keyup', this.on_emoji_keyup, false);
        });
    }

    trap_focus(e) {
        if (!this.dialog.contains(e.target)) {
            if (e.target.nextElementSibling !== null) {
                this.dialog.querySelector('[role="tabpanel"]:not([hidden]) button').focus();
            } else {
                this.dialog.querySelector('button[aria-selected="true"]').focus();
            }
        }
    }

    on_click(e) {
        const target = e.target;
        if (target.parentElement.classList.contains('emoji-display')) {
            let variations = target.dataset.variations;
            if (variations) {
                variations = variations.split('-');
                variations.unshift(target.textContent);
                let component_picker = new EmojiComponent(variations, target);
                component_picker.on_choose = emoji => {
                    this.on_choose(emoji);
                    component_picker.close();
                }
            } else {
                this.on_choose(e.target.textContent);
            }
        }
    }

    on_tab_click(e) {
        this.activate_tab(e.target, false);
    }

    on_tab_keydown(e) {
        let key = e.key;
        switch (key) {
            case 'Home':
                let first = this.dialog.querySelector('[role="tab"]');
                this.activate_tab(first, true);
                e.preventDefault();
                break;
            case 'End':
                let last = this.dialog.querySelector('[role="tab"]:last-of-type');
                this.activate_tab(last, true);
                e.preventDefault();
                break;
        }
    }

    on_tab_keyup(e) {
        let key = e.key;
        switch (key) {
            case 'ArrowLeft':
                let previous = e.target.previousElementSibling
                    || e.target.parentElement.lastElementChild;
                this.activate_tab(previous, true);
                break;
            case 'ArrowRight':
                let next = e.target.nextElementSibling
                    || e.target.parentElement.firstElementChild;
                this.activate_tab(next, true);
                break;
        }
    }

    activate_tab(tab, set_focus) {
        this.deactivate_tab(this.active_tab);

        tab.removeAttribute('tabindex');
        tab.setAttribute('aria-selected', 'true');
        let panel = tab.getAttribute('aria-controls');
        document.getElementById(panel).removeAttribute('hidden');
        if (set_focus) {
            tab.focus();
        }
    }

    deactivate_tab(tab) {
        tab.setAttribute('tabindex', '-1');
        tab.setAttribute('aria-selected', 'false');
        let panel = tab.getAttribute('aria-controls');
        document.getElementById(panel).setAttribute('hidden', 'hidden');
    }
}

class EmojiComponent extends EmojiDialog {
    constructor(components, lastFocused) {
        super(document.getElementById('emoji-component-parent'),
            document.getElementById('emoji-component'), lastFocused);

        this.fill(components);
        this.open();
    }

    fill(components) {
        const container = this.dialog.querySelector('.emoji-display');
        container.innerHTML = '';
        components.forEach((component) => {
            const button = document.createElement('BUTTON');
            button.textContent = component;
            container.appendChild(button);
        });
    }

    open() {
        super.open();

        const panel = this.dialog.querySelector('.emoji-display');
        panel.addEventListener('keydown', this.on_emoji_keydown, true);
        panel.addEventListener('keyup', this.on_emoji_keyup, false);

        this.dialog.querySelector('button').focus();
    }

    close() {
        super.close();

        const panel = this.dialog.querySelector('.emoji-display');
        panel.removeEventListener('keydown', this.on_emoji_keydown, true);
        panel.removeEventListener('keyup', this.on_emoji_keyup, false);
    }

    on_click(e) {
        if (e.target.parentElement.classList.contains('emoji-display')) {
            this.on_choose(e.target.textContent);
        }
    }

    on_emoji_keydown(e) {
        super.on_emoji_keydown(e);

        if (e.key === 'Tab') {
            e.preventDefault();
        }
    }

    trap_focus(e) {

    }
}
